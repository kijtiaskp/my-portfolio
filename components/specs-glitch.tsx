...
export const SpecsGlitch = ({ className = "" }: SpecsGlitchProps) => {
  ...
  const triggerCascadingGlitch = useCallback((changeText = false) => {
  ...
  useEffect(() => {
    ...
    // Random periodic glitches during reading periods
    const randomInterval = setInterval(() => {
      if (Math.random() < 0.6) { // 60% chance of random glitch during reading
        triggerRandomGlitch()
      }
    }, 800) // Check every 0.8 seconds for more frequent glitches
